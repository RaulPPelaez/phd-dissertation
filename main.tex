...
\begin{frame}
  ...
  \only<+>{\includesvg[width=0.8\linewidth]{gfx/celllist3}}%
  \note<1>{Let us by virtually binning the domain, where each cell is identified by unique coordinates.}
  ...
  \note<3>{For any set of random particles, we hash and sort them along the curve. Before this sorting, there is no correlation between a particle's location in memory and space.}
...
% Maybe more intiutively, the force coming from the confining potential, induces a flow which acts as a source of fluid momentum in the plane. And in fact enhances collective diffussion.

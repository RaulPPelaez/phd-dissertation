    ...
    \begin{column}{0.5\linewidth}
      ...
      \begin{overlayarea}{\linewidth}{0.5\paperheight}
        \only<1-3>{\includesvg[width=0.9\linewidth]{gfx/q2D}\\
          \begin{center}
            q2D: $\delta\rightarrow\infty$. 3D fluid, 2D particles
          \end{center}
          }
      ...
      \begin{column}{0.63\linewidth}
        \onslide<3->{
          ...
          \begin{overlayarea}{\linewidth}{0.3\paperheight}
            ...
            \only<7>{
              \begin{equation*}
                \tens{M}_{ij}=\iint{\delta(\vec{r}-\vec{\ppos}_j)\oper{G}(\vec{r},\vec{r}^\prime)\delta(\vec{\ppos}_i-\vec{r}^\prime)d\vec{r}^\prime d\vec{r}}
              \end{equation*}
            }%
...
\begin{frame}
  ...
  \begin{tikzpicture}[thick]
    ...
    \onslide<2>{
      \node [style = {draw}] (b) at (0, 0) {$\vec{u}_i = \oper{J}_{\vec{\ppos}_i}\mathfrak{F}^{-1}\fou{\oper{G}}(\mathfrak{F}\oper{S}\vec{F} + \vec{k}\cdot\mathcal Z)$};
...
\begin{frame}
  ...
  \begin{block}{\centering Triply Periodic}
...
\begin{frame}
  ...
  \begin{overlayarea}{\linewidth}{0.2\paperheight}
    \begin{block}{\centering Force Coupling Method}
      \begin{center}
        \only<1-8>{$\vec{u}_i = \oper{J}_{\vec{\ppos}_i}\mathfrak{F}^{-1}\fou{\oper{G}}_{\text{3D}}(\mathfrak{F}\oper{S}\vec{F} + \vec{k}\cdot\mathcal Z)$}%
        \only<9-10>{$\vec{u}_i = \oper{J}_{\vec{\ppos}_i}\mathfrak{F}^{-1}\left(\fou{\oper{G}}_{\text{3D}}\mathfrak{F}\oper{S}\vec{F} + \fou{\vec{w}}\right)$}
        \only<11->{$\vec{u}_i = \oper{J}_{\vec{\ppos}_i}\mathfrak{F}^{-1}\left(\fou{\oper{G}}_{\qtd}\mathfrak{F}\oper{S}\vec{F} + \fou{\vec{w}}\right)$}
      \end{center}
    \end{block}
  \end{overlayarea}
  \begin{columns}[T]
    ...
    \begin{column}{0.6\linewidth}
      \begin{overlayarea}{\linewidth}{0.7\paperheight}
        \only<1-7>{
          ...
          \only<5->{Solutions:}
          \begin{enumerate}
          \item<6-> Pure 2D description 
          \item<7> Preconvolve $\oper{G}$ in Z
          \end{enumerate}
        }
        \only<8-9>{
          Pure 2D description:
        ...
        \only<10->{
          Preconvolve $\oper{G}$ in Z
          \begin{block}{Remember}
            $\delta(\vec{r}) := \phi(r_x)\phi(r_y)\phi(r_z)\rightarrow$ Smeared delta
          \end{block}
        }
        \only<11>{
          \begin{center}
    ...
    \begin{columns}[T]
      \begin{column}{0.4\linewidth}
        \begin{block}<2->{Spreading}
          $\oper{S}(\vec{r})\vec{Q} = \sum_i Q_i \delta(\vec{\ppos}_i - \vec{r})$
        \end{block}
        \begin{block}<3->{Gaussian sources}
          $\delta(\vec{r}) = \frac{1}{(2\pi a^2)^{3/2}} e^{-r^2/(2a^2)}$
        \end{block}
